...
def _save_transformed_metadata(pd_raw_df: pd.DataFrame):
    ...
    print(f"🗑️ Deleted existing ORC file: {file_path}")
    selected_df.to_orc(file_path, engine="pyarrow")
    print(f"✅ The data of {metadata_table} was transformed")


def _save_transformed_hourly_data(pd_raw_df: pd.DataFrame):
    selected_df = pd_raw_df[["request_id", "time", "temperature_2m", "windspeed_10m", "precipitation"]]
    ...
    selected_df.to_orc(file_path, engine="pyarrow")
    print(f"✅ The data of {hourly_table} was transformed")
